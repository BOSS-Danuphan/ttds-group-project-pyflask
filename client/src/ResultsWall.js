...
class ResultsWall extends Component {
    ...
    render() {
        const tweetList = this.props.stTweets.map((tweet, index) => <li key={tweet}><TwitterTweetEmbed tweetId={tweet}/></li>);
        const rtTweetList= this.props.rtTweets.map((tweet, index) => <li key={tweet}><TwitterTweetEmbed tweetId={tweet}/></li>);
        return (
